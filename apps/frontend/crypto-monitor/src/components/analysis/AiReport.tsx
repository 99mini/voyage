    ...
    <div className={`${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white'} p-6 rounded-lg shadow-md`}>
      ...
      <div className={`prose prose-sm max-w-none ${theme === 'dark' ? 'prose-invert' : ''}`}>
        <ReactMarkdown
          components={{
            h1: ({ children }) => (
              <h1 className={`text-2xl font-bold my-4 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                {children}
              </h1>
            ),
            h2: ({ children }) => (
              <h2 className={`text-xl font-bold my-3 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                {children}
              </h2>
            ),
            h3: ({ children }) => (
              <h3 className={`text-lg font-bold my-2 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                {children}
              </h3>
            ),
            p: ({ children }) => (
